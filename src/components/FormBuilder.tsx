...
type PreviewItem = {
    id: string; qid: string; width: 0 | 1 | 2 | 3; // 3->12, 2->8, 0->6, 1->4
};
...
export default function FormBuilder({ slug }: { slug?: string }) {
    ...
    function onDragEnd(e: DragEndEvent) {
        ...
        // Normalize: accept either divider-# or dropzone-# (if you experimented earlier)
        const isDivider = toId.startsWith('divider-') || toId.startsWith('dropzone-');
        const k = isDivider
            ? parseInt(toId.replace('divider-', '').replace('dropzone-', ''), 10)
            : null;

        // ===== PREVIEW/BANK → DIVIDER (explicit new row at index k) =====
        if (isDivider && k !== null && !Number.isNaN(k)) {
            // From PREVIEW → DIVIDER
            if (previewIds.includes(fromId)) {
        ...
        // ===== PREVIEW → PREVIEW (card on card: swap/reorder within row, keep width) =====
        if (previewIds.includes(fromId) && (previewIds.includes(toId) || toId === PREVIEW_ID)) {
            ...
            setPreview(p => {
                // remove dragged first so indices reflect destination
                const without = [...p];
                const [dragged] = without.splice(oldIndex, 1);
                if (!dragged) return p;
                ...
                // reinsert within the same flow; no forced 12, swap both directions
    ...
    return (
        <div className="container py-3">
            {/* Local styles: row divider (soft blue dotted), hover highlight */}
            <style jsx global>{`
  .ff-row-divider {
    height: 14px; /* taller so it feels like a real drop slot */
    margin: 8px 0;
    border-top: 2px dotted transparent; /* hidden until active */
    background: transparent;
    transition: all .18s ease-in-out;
  }
  .ff-row-divider.ff-over {
    border-top-color: #0d6efd;
    background: rgba(13, 110, 253, 0.08); /* soft blue halo */
    opacity: 1;
    transform: scaleY(1.2); /* make it feel like it expands */
  }
`}</style>
                                                    ...
                                                    <DraggableBank id={item.id}>
                                                        <div className="d-flex align-items-center justify-content-between">
                                                            <div>
                                                                ...
                                                                <small className="text-muted">{item.type}{item.required ? ' • required' : ''}</small>
                                                            </div>
                                                        </div>
                                                    </DraggableBank>
                                                </li>
                                ...
                                <div className="card-body">
                                    <Droppable id={PREVIEW_ID}>
                                        <SortableContext items={previewIds} strategy={rectSortingStrategy}>
                                                                ...
                                                                <div key={pi.id} className={`col-12 col-md-${col}`} data-preview-id={pi.id}>
                                                                    <SortablePreview id={pi.id}>
                                                                        <PreviewCard
                                                                            ...
                                                                            width={pi.width}
                                                                            onSetWidth={(w) => {
                                                                                // free width changes; can validate on save if desired
                                            ...
                                            {/* If there are no rows/items, still show the divider and an empty hint */}
                                            {preview.length === 0 && (
                                ...
                                {preview.map((pi, idx) => {
                                    ...
                                    return (
                                        <div key={`${pi.id}_${idx}`} className={`col-12 col-md-${col}`} data-preview-id={pi.id}>
                                            <label className="form-label mb-1">{qn.label}{qn.required ? ' *' : ''}</label>
                                            <input className="form-control" placeholder="Preview" disabled />
                                        </div>
                                    );
                                })}
                            </div>
